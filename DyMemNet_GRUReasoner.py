...
class Classifier(nn.Module):
    ...
    def forward(self, question_text, candidate_text, sci_text, knowledge_text, user_annotations):
        ...
        if len(user_annotations[1])>0:
            for single_fact in user_annotations[1]:
                knowledge_fact.append(self.inputModule.forward(single_fact.split(' ')))
    ...
    def get_loss_interactive(self, outputs, targets, att_score, annotated_sen):
        ...
        print('length of annotate:', len(annotated_sen[0]))
        
        selected_index = selection_loss_python_list.index(min(selection_loss_python_list))
        _, preds=torch.max(outputs.view(1,4), dim=1)
        #preds = F.softmax(output, dim=1)
        #_, pred_ids = torch.max(preds, dim=1)
        #return prediction_loss+selection_loss, pred_ids
        
        return selection_loss_torch_list[selected_index], preds
...
class DyMemNet_Trainer():
    ...
    def train_interactive_forward(self, instance, choice_id, user_annotations):
